...
public class WeeklyReportInstanceWise {
	static Connection connection = null;
	private static String filePath;
	static HashMap<String, String> details;
//	private static String instanceShortName = "Utkarsh";
	...
	public static void main(String[] args) {
		...
		try {
			...
			details = readDetails();
			String instanceshortName = details.get("instanceName");
			String[] instanceNamelist = { instanceshortName };
			if (instanceshortName.contains(",")) {
				instanceNamelist = instanceshortName.split(",");
			}

			filePath = createFilePath(instanceshortName);
			String mainQuery = createQuery(instanceshortName, details.get("fromDate"), details.get("toDate"));

//						System.out.println("*******************************");
//						System.out.println("Your query \n" + mainQuery);
//						System.out.println("*******************************");
			System.out.println("Please wait," + instanceshortName + "  query is executing.....");
			...
			int pageCount;
			int successPageCount = 0;
			int misAdditionalPageCount = 0;
			String instanceName;
...
//						csvWrite(filePath, instanceName, zone, frequency, desireCount, runcount, successCount,
//								misAdditional, pageCount, successPageCount, misAdditionalPageCount);

					csvWrite(filePath, instanceName, "instanceShortName", frequency, desireCount, runcount,
							successCount, misAdditional, pageCount, successPageCount, misAdditionalPageCount);
				} catch (ArrayIndexOutOfBoundsException e) {
					e.printStackTrace();
				}
			}
			System.out.println(instanceshortName + " Zone successfully generated! \n");
	...
	@SuppressWarnings("unused")
	public static String createQuery(String instanceShortName, String fromDate, String toDate) {
...
//				System.out.println("toDate ->"+toDate);
		String query = "SELECT  \n" + "	 mi.monitoring_instance_id,\n" + "    mi.instance_name AS instance_name,  \n"
				...
				+ "    mi.instance_name LIKE \"%" + instanceShortName + "%\" \n" + "    AND mi.is_active = 'Y'\n"
	...
	public static HashMap<String, String> readDetails() {
		...
		details.put("instanceName", instanceName);
...
//						System.out.println(ldfromDate + "--------------" + fullDayResultSet.getString("schedules"));
	...
	public static void csvWrite(String csvFilePath, String instance_name, String zone, int frequency, int desireCount,
			int runcount, int successCount, int misAdditional, int desired_pageCount, int success_pageCount,
			int missed_additional_pagecount) throws IOException {
		...
		try {
			...
			if (!fileExists) {
				out.println("instance_name,application_freq,Desired_Count,Run_Count,success_count,Missed_Additional");
			}
			// Append data safely with quotes (avoid comma issues)
			out.println(
					String.join(",", "\"" + instance_name + "\"", "\"" + frequency + "\"", "\"" + desireCount + "\"",
							"\"" + runcount + "\"", "\"" + successCount + "\"", "\"" + misAdditional + "\""));
			out.close();
			bw.close();
			fw.close();
		} catch (Exception e) {
...
//	public static void csvWriteold(String csvFilePath, String instance_name, String zone, int frequency, int desireCount,
//			int runcount, int successCount, int misAdditional, int desired_pageCount, int success_pageCount,
//			int missed_additional_pagecount) throws IOException {
//		File csvFile = new File(csvFilePath);
//		boolean fileExists = csvFile.exists();
...
//			if (!fileExists) {
//				out.println(
//						"instance_name,zone,application_freq,Desired_Count,Run_Count,success_count,Missed_Additional,desired_pageCount,success_pageCount,missed_additional_pagecount");
//			}
//			// Append data safely with quotes (avoid comma issues)
//			out.println(String.join(",", "\"" + instance_name + "\"", "\"" + zone + "\"", "\"" + frequency + "\"",
//					"\"" + desireCount + "\"", "\"" + runcount + "\"", "\"" + successCount + "\"",
//					"\"" + misAdditional + "\"", "\"" + desired_pageCount + "\"", "\"" + success_pageCount + "\"",
//					"\"" + missed_additional_pagecount + "\""));
//			out.close();
//			bw.close();
//			fw.close();
//		} catch (Exception e) {
